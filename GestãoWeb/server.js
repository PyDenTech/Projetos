...
app.get('/localizar', (req, res) => {
    res.sendFile(path.join(__dirname, 'views', 'desenhar-rota.html'));
});
...
app.post('/webhook', async (req, res) => {
    ...
    if (data.object && data.entry && data.entry[0].changes && data.entry[0].changes[0].value.messages) {
        ...
        if (userState[senderNumber] && userState[senderNumber].step === 'enviar_localizacao') {
            if (location) {
                userState[senderNumber].latitude = location.latitude;
                userState[senderNumber].longitude = location.longitude;

                // Usa as coordenadas para buscar o ponto de parada mais próximo
                const nearestStop = await getNearestStop({ lat: location.latitude, lng: location.longitude });

                if (nearestStop) {
                    const directionsUrl = `https://www.google.com/maps/dir/?api=1&origin=${location.latitude},${location.longitude}&destination=${nearestStop.latitude},${nearestStop.longitude}&travelmode=walking`;
                    await sendTextMessage(
                        senderNumber,
                        `O ponto de parada mais próximo é o ${nearestStop.nome}, localizado em: ${nearestStop.descricao}. Coordenadas: ${nearestStop.latitude}, ${nearestStop.longitude}.\n\nClique no link para ver a rota a pé até o ponto de parada: [Traçar Rota no Google Maps](${directionsUrl})`
                    );
                } else {
                    await sendTextMessage(senderNumber, 'Não foi possível encontrar um ponto de parada próximo com base na localização enviada.');
                }

                delete userState[senderNumber]; // Reseta o estado do usuário após completar o processo
            } else {
                await sendTextMessage(senderNumber, 'Por favor, envie sua localização atual para continuarmos.');
            }
        }
...
async function checkStudentTransport(to) {
    ...
    if (aluno) {
        if (aluno.usa_transporte_escolar) {
            // Tenta converter o endereço do aluno em coordenadas usando a API do Google Maps
            ...
            if (coordinates) {
                ...
                // Se não for possível converter o endereço em coordenadas, pede a localização manual
                userState[to].step = 'enviar_localizacao';
                await sendTextMessage(
                    to,
                    'Não foi possível identificar as coordenadas do endereço fornecido. Por favor, envie sua localização atual *somente se estiver em sua residência*, para que possamos localizar o ponto de parada mais próximo de maneira precisa.'
                );
            }
        } else {
...
// Endpoint para buscar dados de localização filtrados
app.get('/api/get-location-data', async (req, res) => {
    const { busPlate, routeId, date } = req.query;

    try {
        // Montar a query com base nos filtros recebidos
        let query = `SELECT latitude, longitude, time FROM gps_data WHERE 1=1`;
        const values = [];

        if (busPlate) {
            query += ` AND bus_plate = $${values.length + 1}`;
            values.push(busPlate);
        }
        ...
        if (date) {
            query += ` AND DATE(time) = $${values.length + 1}`;
            values.push(date);
        }
        ...
        console.error(err);
        res.status(500).send('Erro ao buscar dados de GPS');
    }
});

// Endpoint para buscar arquivos GPX filtrados
app.get('/api/get-gpx-file', async (req, res) => {
    const { busPlate, routeId, date } = req.query;
    ...
    try {
        ...
        if (date) {
            query += ` AND DATE(created_at) = $${values.length + 1}`;
            values.push(date);
        }
        ...
        if (result.rows.length === 0) {
            return res.status(404).send('Arquivo GPX não encontrado');
        }
        ...
        console.error(err);
        res.status(500).send('Erro ao buscar arquivo GPX');
    }
});
